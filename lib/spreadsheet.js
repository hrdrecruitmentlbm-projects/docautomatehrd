...
export function mapMasterRows(headers, rows) {
  ...
  // TEMPAT / TANGGAL LAHIR: bisa 2 kolom ("tempat", "tanggal lahir")
  // atau 1 header gabungan yang menempati 2 kolom (kolom ke-2 kosong).
  let tempatIdx = findCol(hm, ["tempat"]);
  let tglLahirIdx = findCol(hm, ["tanggal lahir"]);
  if (tempatIdx >= 0 && tglLahirIdx < 0) {
    const raw = String(headers[tempatIdx] || "").toLowerCase();
    if (raw.includes("tempat") && raw.includes("tanggal")) {
      // header gabungan: tanggal lahir ada di kolom sebelahnya
      tglLahirIdx = tempatIdx + 1;
    }
  }
  if (tempatIdx < 0) tempatIdx = findCol(hm, ["tempat tanggal lahir"]);
  if (tglLahirIdx < 0) {
    // fallback: kolom berisi tanggal setelah kolom tempat
    tglLahirIdx = -1;
  }
