...
export function buildPkwtReplacements({ employee = {}, payroll = {}, manual = {}, documentNumber = "", companyLegal = "" }) {
  ...
  const gapok = formatRp(payroll.gapok);
  const uangMakan = formatRp(payroll.u_makan);
  const uangTransport = formatRp(payroll.u_transport);
  const penyesuaian = formatRp(payroll.penyesuaian);
  const tJabatan = formatRp(payroll.t_jabatan);
  const tFungsional = formatRp(payroll.t_fungsional);
  const tKesehatan = formatRp(payroll.t_kesehatan);
  const tTransport = formatRp(payroll.t_transport);
  ...
  return {
    ...
    // {{kop}} tidak pernah diganti KOP gambar — KOP sudah ditempel manual per
    // template. Key kosong ini hanya agar sisa teks {{kop}} hilang, bukan bocor.
    kop: "", KOP: "",
