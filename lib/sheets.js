...
export function quoteTab(tabTitle) {
  return `'${String(tabTitle).replace(/'/g, "''")}'`;
}

// Bounded range keeps big workbooks fast (Vercel Hobby kills slow functions).
export async function readRangeValues(accessToken, spreadsheetId, range) {
  const sheets = getSheetsClient(accessToken);
  try {
    const res = await withRetry(() =>
...
export async function readTabValues(accessToken, spreadsheetId, tabTitle, rangeEnd = "AZ2000") {
  return readRangeValues(accessToken, spreadsheetId, `${quoteTab(tabTitle)}!A1:${rangeEnd}`);
}
